...
@Repository
public interface LecturerReportRepository extends JpaRepository<Student, Long> {

    @Query(value = """
        SELECT 
            s.student_code AS studentCode,
            u.full_name AS studentName,
            COALESCE(vas.present_count, 0) AS presentCount,
            COALESCE(vas.absent_count, 0) AS absentCount,
            COALESCE(vas.excused_count, 0) AS excusedCount,
            COALESCE(vas.late_count, 0) AS lateCount,
            COALESCE(vas.left_early_count, 0) AS leftEarlyCount,
            COALESCE(vas.total_sessions, 0) AS totalSessions,
            COALESCE(vas.attendance_rate, 100.0) AS attendanceRate
        FROM student s
        JOIN user u ON s.user_id = u.id
        LEFT JOIN v_attendance_summary vas ON vas.student_id = s.id 
            AND vas.semester_id = :semesterId
            AND vas.subject_id = :subjectId

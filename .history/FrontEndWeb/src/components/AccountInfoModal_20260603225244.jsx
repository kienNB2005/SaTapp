...
export default function AccountInfoModal({ isOpen, onClose }) {
  ...
  const fetchUserInfo = async () => {
    try {
      ...
      // If API not ready, show error message
      if (err.response?.status === 404 || err.response?.status === 500) {
        setError('API endpoint chưa sẵn sàng. Vui lòng thử lại sau.');
      } else {
        setError('Lỗi khi tải thông tin tài khoản: ' + (err.message || 'Unknown error'));
      }
      console.error(err);
    } finally {
      setLoading(false);

...
export default function AdminTkb() {
  ...
  /**
   * Giữ giá trị filter mới nhất trong ref để fetchSchedules không bị đổi liên tục.
   * Tránh lỗi:
   * - Bấm sang trang bị quay lại trang 1
   * - API bị gọi 2 lần do useEffect chạy lại
   */
  ...
  }, [selectedSemester, searchText, filterDay]);
  ...
  const fetchSchedules = useCallback(async (page = 0) => {
    ...
    try {
      ...
      setCurrentPage(pageData.number ?? pageData.page?.number ?? page);
    } catch (err) {
      setListError(
  ...
  /**
   * Load lại danh sách khi đổi học kỳ hoặc đổi ngày.
   * Không cho searchText vào đây để tránh gọi API 2 lần khi gõ tìm kiếm.
   */
  useEffect(() => {
  ...
  }, [searchText, selectedSemester, view, fetchSchedules]);
  ...
  const renderListErrorInline = () => (
    <div className="atk-list-state atk-list-error">
      ...
      <button className="btn btn-p" onClick={() => fetchSchedules(currentPage)}>
        Thử lại
      </button>
    </div>
  );
            ...
            {schedules.map((item) => (
              <tr key={item.id}>
                ...
                <td className="atk-center-cell">
                  <div className="atk-session-count">{item.totalSessions}</div>
                </td>
              </tr>
            ))}
          </tbody>

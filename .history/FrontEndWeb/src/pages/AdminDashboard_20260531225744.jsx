  ...
  return (
    <div className="card">
      ...
      <div style={{ padding: '20px', position: 'relative' }}>
        <svg width="100%" height={chartHeight + 40} viewBox={`0 0 ${chartWidth + 40} ${chartHeight + 40}`} style={{ minWidth: '100%' }}>
          {/* Y-axis labels */}
          {[0, 25, 50, 75, 100].map((val) => (
            <g key={`y-${val}`}>
              <text x="10" y={chartHeight - (val / 100) * chartHeight + 5} fontSize="10" fill="var(--tx3)" textAnchor="end">
                {val}
              </text>
              <line x1="15" y1={chartHeight - (val / 100) * chartHeight} x2={chartWidth + 20} y2={chartHeight - (val / 100) * chartHeight} 
                    stroke="var(--bd)" strokeWidth="0.5" strokeDasharray="2,2" opacity="0.3" />
            </g>
          ))}

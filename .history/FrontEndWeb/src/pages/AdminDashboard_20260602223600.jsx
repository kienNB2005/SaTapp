...
function WeeklyAttendanceChart({ data, hoveredDay, setHoveredDay }) {
  ...
  const viewBoxWidth = chartWidth + chartRightPadding;
          ...
          {data.map((item, idx) => {
            ...
            return (
              <g key={`bar-${idx}`}>
                ...
                {/* Day label */}
                <text x={x + barWidth + 3} y={topPadding + chartHeight + 20} fontSize="12" fill="var(--tx2)" textAnchor="middle" fontWeight="600">
                  {item.day}
                </text>
              </g>
            );
        ...
        <svg width="100%" height={viewBoxHeight} viewBox={`0 0 ${viewBoxWidth} ${viewBoxHeight}`} style={{ minWidth: '100%' }}>
          ...
          {points.map((point, idx) => (
            <g key={`point-${idx}`}>
              ...
              />
              <text x={point.x} y={topPadding + chartHeight + 20} fontSize="11" fill="var(--tx2)" textAnchor="middle" fontWeight="600">
                {point.month}
              </text>
            </g>
          ))}

...
import {
  ...
  ArrowLeft, Download, RefreshCw, Search, Users, UserPlus, Eye, Filter, ChevronLeft, ChevronRight, X
} from 'lucide-react';
...
export default function AdminStudents() {
  ...
  const [addForm, setAddForm] = useState({ fullName: '', email: '', studentCode: '', adminClassId: '', phoneNumber: '', gender: '', birthday: '', birthPlace: '' });
  const [addLoading, setAddLoading] = useState(false);
  const [addError, setAddError] = useState('');

  // --- EDIT STATE ---
  const [editingStudent, setEditingStudent] = useState(null);
  const [editForm, setEditForm] = useState({ fullName: '', email: '', isActive: true, phoneNumber: '', gender: '', birthday: '', birthPlace: '' });
  const [editLoading, setEditLoading] = useState(false);
  const [editError, setEditError] = useState('');

  // --- IMPORT STATE ---
  const [importStep, setImportStep] = useState('upload'); // upload, importing_preview, preview, importing_confirm, success, error
  ...
  const fetchStudents = async () => {
    ...
    try {
      ...
      console.error(err);
      setListError(err.response?.data?.message || err.message || JSON.stringify(err) || 'Không thể tải danh sách sinh viên.');
    } finally {
  ...
  const handleEditClick = (student) => {
    ...
    setEditForm({
      ...
      email: student.email || '',
      isActive: student.isActive !== false, // default true unless explicitly false
  ...
  const handleAddStudentClick = () => {
    ...
    setAddForm({ fullName: '', email: '', studentCode: '', adminClassId: '', phoneNumber: '', gender: '', birthday: '', birthPlace: '' });
    setAddError('');
  };

  const handleCreateStudent = async () => {
    if (!addForm.fullName || !addForm.email || !addForm.studentCode || !addForm.adminClassId) {
      setAddError('Vui lòng điền đầy đủ Tên, Email, Mã sinh viên và chọn Lớp hành chính');
      return;
    }
    setAddLoading(true);
    setAddError('');
  ...
  const handleConfirmImport = async () => {
    const validStudents = previewList.filter(item => item.valid).map(item => item.student);
    ...
    <div className="page active">
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '20px' }}>
        ...
        <div style={{ display: 'flex', gap: '10px' }}>
          {view === 'list' ? (
            <>
              <button className="btn btn-s" onClick={() => { setPage(0); fetchStudents(); }} style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                <RefreshCw size={14} /> Làm mới
              </button>
              <button className="btn btn-p" onClick={handleAddStudentClick} style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
              ...
              <button className="btn btn-s" onClick={() => setView('import')} style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                <UploadCloud size={14} /> Import Sinh Viên
              </button>
            </>
          ) : (
          ...
          {importStep === 'upload' && (
            <div className="g2">
              ...
              <div className="card" style={{ background: 'linear-gradient(135deg, var(--bg3), var(--bg2))', display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', padding: '30px', textAlign: 'center' }}>
                ...
                <h3 style={{ fontSize: '16px', marginBottom: '10px' }}>File mẫu Import Sinh Viên</h3>
                <p style={{ fontSize: '13px', color: 'var(--tx3)', lineHeight: '1.6', maxWidth: '300px' }}>
                  Hệ thống sẽ tự động <strong>thêm mới</strong> những Sinh viên chưa có và <strong>cập nhật</strong> những Sinh viên đã tồn tại dựa trên mã Sinh viên. Sinh viên phải thuộc một <strong>Lớp hợp lệ</strong>.
                </p>
        ...
        <div className="card">
            ...
            <div style={{ display: 'flex', gap: '10px', alignItems: 'center', flexWrap: 'wrap' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '6px', background: 'var(--bg2)', padding: '6px 12px', borderRadius: '8px', border: '1px solid var(--bd)' }}>
                ...
                <select
                  ...
                  onChange={e => { setPage(0); setSelectedClassId(e.target.value); }}
                >
          ...
          {listLoading ? (
            ...
            <div style={{ padding: '40px', textAlign: 'center' }}>
              ...
              <button className="btn btn-p" onClick={() => fetchStudents()}>Thử lại</button>
            </div>
          ) : students.length === 0 ? (
            <div style={{ padding: '60px', textAlign: 'center' }}>
                    ...
                    {students.map((student, idx) => (
                      <tr key={student.id}>
                        ...
                        <td>
                          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                            <div style={{
                              ...
                              {student.fullName ? student.fullName.substring(0, 2).toUpperCase() : 'SV'}
                            </div>
                            <span style={{ fontWeight: '500' }}>{student.fullName}</span>
                          </div>
                        </td>
                        <td style={{ fontSize: '12px', color: 'var(--tx3)' }}>{student.email}</td>
                        <td style={{ fontSize: '12px', color: 'var(--tx3)' }}>{student.phoneNumber || '—'}</td>
                        <td style={{ fontSize: '12px' }}>
                          {student.gender === 'male' ? 'Nam' : student.gender === 'female' ? 'Nữ' : student.gender === 'other' ? 'Khác' : '—'}
                        </td>
                        <td>
                          <div style={{ fontSize: '12px', fontWeight: '500' }}>{student.adminClassCode || '—'}</div>
                          <div style={{ fontSize: '11px', color: 'var(--tx3)' }}>{student.adminClassName}</div>
                        </td>
                        <td>
                          {student.isActive ? (
                        ...
                        <td style={{ textAlign: 'right' }}>
                          <button className="btn btn-s btn-sm" onClick={() => handleEditClick(student)}>Sửa</button>
                        </td>
                      </tr>
                    ))}
                  </tbody>
        ...
        <div style={{
          ...
          <div className="card" style={{ width: '400px', padding: '24px', position: 'relative', boxShadow: '0 10px 25px rgba(0,0,0,0.1)' }}>
            ...
            <h3 style={{ marginBottom: '20px', fontSize: '18px', display: 'flex', alignItems: 'center', gap: '8px' }}>
              <Users size={20} color="var(--bl)" /> Chỉnh sửa Sinh viên
            </h3>
            ...
            <div style={{ display: 'flex', gap: '10px', justifyContent: 'flex-end' }}>
              <button className="btn btn-s" onClick={() => setEditingStudent(null)}>Hủy</button>
              <button className="btn btn-p" onClick={handleUpdateStudent} disabled={editLoading}>
      ...
      {isAddingStudent && (
        <div style={{
          ...
          <div className="card" style={{ width: '400px', padding: '24px', position: 'relative', boxShadow: '0 10px 25px rgba(0,0,0,0.1)' }}>
            ...
            <h3 style={{ marginBottom: '20px', fontSize: '18px', display: 'flex', alignItems: 'center', gap: '8px' }}>
              <UserPlus size={20} color="var(--bl)" /> Thêm mới Sinh viên
            </h3>
            ...
            <div style={{ marginBottom: '16px' }}>
              <label style={{ display: 'block', marginBottom: '6px', fontSize: '13px', fontWeight: '600' }}>Mã sinh viên</label>
              <input
                type="text"
            ...
            <div style={{ marginBottom: '24px' }}>
              <label style={{ display: 'block', marginBottom: '6px', fontSize: '13px', fontWeight: '600' }}>Lớp hành chính</label>
              <select
                value={addForm.adminClassId}
                onChange={e => setAddForm({ ...addForm, adminClassId: e.target.value })}
                style={{ width: '100%', padding: '10px', borderRadius: '8px', border: '1px solid var(--bd)', outline: 'none' }}
              >
            ...
            <div style={{ display: 'flex', gap: '10px', justifyContent: 'flex-end' }}>
              <button className="btn btn-s" onClick={() => setIsAddingStudent(false)}>Hủy</button>
              <button className="btn btn-p" onClick={handleCreateStudent} disabled={addLoading}>
